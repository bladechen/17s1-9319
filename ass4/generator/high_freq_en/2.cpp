...
std::string fuck_stem(std::string& c)
{
...
int main()
{
    ...
    map<std::string, int> stop;
	std::ifstream infile22("./stop.words");
	std::string line;
    ...
    map<std::string, int> bank;
	std::ifstream infile("./word_bank");
	while (std::getline(infile, line))
	{
		bank[line] = 1;
	}

    vector<std::string> ssss;

	static char buf[14096];
	std::ifstream infile1("words_relation");
	while (std::getline(infile1, line))
	{
		strncpy(buf, line.c_str(), 14095);
        buf[strcspn(buf, "\n")] = 0;
        vector<std::string> cao = split(buf, " ,");
        string key = fuck_stem(cao[0]);
        vector <std::string> rr;
        for (int i = 1; i < cao.size(); ++ i)
        {
            string v = fuck_stem(cao[i]);
            if (v == key || stop.find(v) != stop.end() || bank.find(cao[i]) == bank.end() )
            {
                continue;
            }
            rr.push_back(v);
		...
		// string tmp(buf, k);
        // if (stop[tmp] == 1 || stop [buf] ==1)
        // {
        //     // printf ("this is stop words: %s\n", line.c_str());
        //     continue;
        // }
        // ssss.push_back(buf);
    ...
    // prinf ("mmmmmmmmmmmmmmmmmmmmmmmmmmm\n");
